...
<div class="modal-dialog" style="overflow-y:auto;height: 100%;width: 55%;">
  <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" id="add_minimumstay" name="add_minimumstay" enctype="multipart/form-data"> 
        ...
        <div class="row">
          ...
          <div class="form-group col-md-4">
                <label>Days</label>
                <input type="number" class="form-control" id="minDay" name="minDay"  value="<?php echo isset($id) ? $view[0]->minDay : '' ?>">
              <p class="text-danger er-msg hide minDay-err">required</p>
            </div>
        </div>
        </form>
      </div>
       <div class="modal-footer">
        <?php if(isset($id) && $id!="") { ?>
          <button type="button" class="btn-sm btn-success" name="minimumstay_submit" id="minimumstay_submit">Update</button>
        <?php } else { ?>
            <button type="button" class="btn-sm btn-success" name="minimumstay_submit" id="minimumstay_submit">Submit</button> 
        <?php } ?>
...
<script type="text/javascript">
  $(document).ready(function() {
   var nextDay = new Date($("#date_picker1").val());
    ...
    $("#minimumstay_submit").click(function() {
      $(".er-msg").addClass("hide");
      $.ajax({
          dataType: 'json',
          type: "Post",
          url: base_url+'HotelSupplier/minimumstaysubmit',
          data: $("#add_minimumstay").serialize(),
          success: function(data) {
            if (data['status']=="failed") {
              ...
              $("#minimumstay_submit").prop("disabled",true);
              $(".close").trigger("click");
              ...
              var minimumstay_table = $('#minimumstay_table').dataTable({
                "bDestroy": true,
                "ajax": {
                    url : base_url+'HotelSupplier/minimumstaylist/<?php echo $contractid ?>',
                    type : 'GET'
                },
