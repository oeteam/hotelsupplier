...
<div class="modal-dialog" style="overflow-y:auto;height: 100%;width: 55%;">
  <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" id="add_boardsupplement" name="add_boardsupplement" enctype="multipart/form-data"> 
          ...
          <div class="form-group col-md-4">
              ...
              <select name="board" id="board" class="form-control">
                  <?php foreach ($board as $key => $value) { 
                      if(isset($id) && $view[0]->board==$value  ) { ?>
                      <option selected="" value="<?php echo $value ?>"><?php echo $value ?></option>
                  <?php } else { ?>
                      <option value="<?php echo $value ?>"><?php echo $value ?></option>
                  <?php } } ?>
              </select>
              <p class="text-danger er-msg hide board-err">required</p>
          </div> 
          <div class="form-group col-md-4">
        ...
        <div class="row">
          ...
          <div class="form-group col-md-4">
              ...
              <input type="number" class="form-control" name="StartAge" id="StartAge" value="<?php echo isset($id) ? $view[0]->startAge : '' ?>">
              <p class="text-danger er-msg hide StartAge-err">required</p>
          </div>
          <div class="form-group col-md-4">
              <label>Final Age</label>
              <input type="number" class="form-control" name="FinalAge" id="FinalAge" value="<?php echo isset($id) ? $view[0]->finalAge : '' ?>">
              <p class="text-danger er-msg hide FinalAge-err">required</p>
          </div>
        </div>
        <div class="row">
          <div class="form-group col-md-4">
              <label>Amount</label>
              <input type="number" class="form-control" name="Amount" id="Amount" value="<?php echo isset($id) ? $view[0]->amount : '' ?>">
       ...
       <div class="modal-footer">
        <?php if(isset($id) && $id!="") { ?>
          <button type="button" class="btn-sm btn-success" name="board_submit" id="board_submit">Update</button>
        <?php } else { ?>
            <button type="button" class="btn-sm btn-success" name="board_submit" id="board_submit">Submit</button> 
        <?php } ?>
   ...
   var nextDay = new Date($("#date_picker1").val());
    ...
    $("#board_submit").click(function() {
      ...
      $.ajax({
          dataType: 'json',
          type: "Post",
          url: base_url+'HotelSupplier/boardsupplementsubmit',
          data: $("#add_boardsupplement").serialize(),
          success: function(data) {
            if (data['status']=="failed") {
              ...
              $("#board_submit").prop("disabled",true);
              $(".close").trigger("click");
              AddToast("success","Updated Successfully","!");
              var boardsupplement_table = $('#boardsupplement_table').dataTable({
                "bDestroy": true,
                "ajax": {
                    url : base_url+'HotelSupplier/boardsupplementlist/<?php echo $contractid ?>',
                    type : 'GET'
                },
