...
<div class="modal-dialog" style="overflow-y:auto;height: 100%;width: 55%;">
  <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" id="add_cancellation" name="add_cancellation" enctype="multipart/form-data"> 
        ...
        <div class="row">
          <div class="form-group col-md-4">
              ...
              <input type="number" class="form-control" name="daysFrom" id="daysFrom" value="<?php echo isset($id) ? $view[0]->daysFrom : '' ?>">
              <p class="text-danger er-msg hide daysFrom-err">required</p>
          </div>
          <div class="form-group col-md-4">
              <label>Days to</label><span>*</span>
              <input type="number" class="form-control" name="daysTo" id="daysTo" value="<?php echo isset($id) ? $view[0]->daysTo : '' ?>">
              <p class="text-danger er-msg hide daysTo-err">required</p>
          </div>
          <div class="form-group col-md-4">
            <label>Application</label>
            <select class="form-control" name="application" id="application">
                <option <?php echo isset($id) && $view[0]->application=='FREE OF CHARGE' ? 'selected' : '' ?> value="FREE OF CHARGE">FREE OF CHARGE</option>
                <option <?php echo isset($id) && $view[0]->application=='STAY' ? 'selected' : '' ?> value="STAY">STAY</option>
                <option  <?php echo isset($id) && $view[0]->application=='FIRST NIGHT' ? 'selected' : '' ?> value="FIRST NIGHT">FIRST NIGHT</option>
            </select>
        ...
        <div class="row">
          <div class="form-group col-md-4">
            ...
            <input type="number" class="form-control" name="CancellationPercentage" id="CancellationPercentage" value="<?php echo isset($id) ? $view[0]->cancellationPercentage : '' ?>">
            <p class="text-danger er-msg hide CancellationPercentage-err">required</p>
          </div>
        </div>
        </form>
      </div>
       <div class="modal-footer">
        <?php if(isset($id) && $id!="") { ?>
          <button type="button" class="btn-sm btn-success" name="cancellation_submit" id="cancellation_submit">Update</button>
        <?php } else { ?>
            <button type="button" class="btn-sm btn-success" name="cancellation_submit" id="cancellation_submit">Submit</button> 
        <?php } ?>
   ...
   var nextDay = new Date($("#date_picker1").val());
    ...
    $("#cancellation_submit").click(function() {
      ...
      $.ajax({
          dataType: 'json',
          type: "Post",
          url: base_url+'HotelSupplier/cancellationsubmit',
          data: $("#add_cancellation").serialize(),
          success: function(data) {
            if (data['status']=="failed") {
              ...
              $("#cancellation_submit").prop("disabled",true);
              $(".close").trigger("click");
              ...
              var cancellation_table = $('#cancellation_table').dataTable({
                  "bDestroy": true,
                  "ajax": {
                      url : base_url+'HotelSupplier/cancellationlist/<?php echo $contractid ?>',
                      type : 'GET'
                  },
